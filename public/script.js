// ===================================================================================
// SCRIPT COMPLETO PARA O DASHBOARD (VERSÃO VERCEL OFICIAL)
// ===================================================================================
...
function initializeCharts() {
    ...
    const commonChartOptions = {
        scales: { x: { type: 'time', time: { unit: 'hour', tooltipFormat: 'dd MMM yyyy, HH:mm', displayFormats: { hour: 'dd/MM HH:mm', day: 'dd/MM/yyyy'}}, title: { display: true, text: 'Data' }}, y: { beginAtZero: false, title: { display: true, }}},
        responsive: true, maintainAspectRatio: false, interaction: { intersect: false, mode: 'index', },
    };
    tempChartInstance = new Chart(tempCtx, { type: 'line', data: { labels: [], datasets: [{ label: 'Temperatura (°C)', data: [], borderColor: '#cf4b00', backgroundColor: 'rgba(207, 75, 0, 0.1)', fill: true, tension: 0.2 }] }, options: { ...commonChartOptions, scales: { ...commonChartOptions.scales, y: { ...commonChartOptions.scales.y, title: { ...commonChartOptions.scales.y.title, text: 'Temperatura (°C)' } } } } });
...
async function updateLatestData() {
    try {
        const response = await fetch('/api/api_latest.php'); // CAMINHO CORRIGIDO
        if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
        const leituras = await response.json();
        atualizarCards(leituras);
        atualizarTabela(leituras);
    } catch (error) { console.error('Falha ao buscar dados recentes:', error); }
}

async function fetchChartData() {
    const startDate = document.getElementById('startDate').value;
    const endDate = document.getElementById('endDate').value;
    let apiUrl = '/api/api_dados.php'; // CAMINHO CORRIGIDO
...
function atualizarCards(leituras) {
    ...
    } else { tempElement.innerText = '-- °C'; humidityElement.innerText = '-- %'; }
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (deleteBtn) {
        deleteBtn.addEventListener('click', async () => {
            ...
            if (confirmado) {
                try {
                    const response = await fetch('/api/delete_all.php', { method: 'POST' }); // CAMINHO CORRIGIDO
                    const result = await response.json();
                    if (response.ok && result.success) {
                        alert('Dados excluídos com sucesso!');
                        updateLatestData();
                        fetchChartData();
                    } else { throw new Error(result.message || 'Falha ao excluir os dados.'); }
                } catch (error) { console.error('Erro ao excluir os dados:', error); alert(`Erro: ${error.message}`); }
            }
        });
    }
});